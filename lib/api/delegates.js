...
module.exports = function (app) {
	 ...
	 * because Lisk Core 3.0 API no longer provides rank as it was removed in
	 * https://github.com/LiskHQ/lisk-sdk/pull/4389
	 */
	...
	function Delegates() {
		this.getAllDelegates = function (limit, offset, cb) {
			request.get({
				url: `${app.get('lisk address')}/delegates/?limit=${limit}&offset=${offset}`,
				json: true,
			}, (err, response, body) => {
	...
	function Active() {
		const getForgedTimeout = 10 * 39;
		this.getActive = function (cb) {
			getDelegatesFromLocalList({ limit: 39 }, (err, response) => {
				if (err) {
					return cb(err || 'Response was unsuccessful');
	...
	function Standby(n) {
		...
		this.actualOffset = (isNaN(this.offset)) ? 39 : this.offset + 39;
	...
	this.getNextForgers = function (temp, error, success) {
		request.get({
			url: `${app.get('lisk address')}/delegates/forgers?limit=39&offset=0`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
